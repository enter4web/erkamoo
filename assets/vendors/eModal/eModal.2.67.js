 ...
 *     size     {string}:          sm || df || lg || xl -> define the modal size.
    ...
    define(['jquery'], function ($, root) {
        ...
        function _getFooter(buttons) {
            ...
            if (buttons) {
                for (var i = 0, k = buttons.length; i < k; i++) {
                    var btnOp = buttons[i];
                    var btn = $('<button>').addClass('btn btn-sm btn-' + (btnOp.style || 'primary'));
        ...
        function _getModalInstance(skipEventChageIfExists) {
            ...
            function createModalElement() {
                return $('<div class="modal fade"><style>.modal-xl{width:96%;}.modal-body{max-height: calc(100vh - 145px);overflow-y: auto;}</style>' +
        ...
        function _setup(params, title) {
            ...
            $ref.find(MODAL_DIALOG)
                .removeClass('modal-sm modal-df modal-lg modal-xl')
